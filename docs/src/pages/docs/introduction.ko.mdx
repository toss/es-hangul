...
es-hangul은 초, 중, 종성 추출과 변환 등 복잡한 한글 문자열 처리를 위한 인터페이스를 제공합니다. JavaScript 기본 메소드와 조합해 처리할 수 있는 문자열 처리는 es-hangul에서 중복으로 제공하지 않고 es-hangul과 JavaScript 기본 기능과 조합해서 처리하면 대부분의 한글 문자열 처리를 기대할 수 있습니다
