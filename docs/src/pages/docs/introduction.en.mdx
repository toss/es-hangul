...
es-hangul provides an interface for complex Korean text processing, such as extracting and converting initial, medial, and final consonants. It does not redundantly offer string processing methods already available in JavaScript, but instead, it can be combined with JavaScript’s built-in functions to handle most Korean text processing tasks.
