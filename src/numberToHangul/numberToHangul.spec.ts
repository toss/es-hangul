...
describe('numberToHangul', () => {
  test('기본 변환', () => {
    ...
    expect(numberToHangul(12345)).toBe('일만이천삼백사십오');
  ...
  test('공백 포함 변환', () => {
    ...
    expect(numberToHangul(12345, true)).toBe('일만 이천삼백사십오');
  ...
  test('0과 10,000보다 작은 경우', () => {
    ...
    expect(numberToHangul(1000)).toBe('천');
    expect(numberToHangul(1001)).toBe('천일');
    expect(numberToHangul(1100)).toBe('천백');
    expect(numberToHangul(1200)).toBe('천이백');
    expect(numberToHangul(1234)).toBe('천이백삼십사');
    expect(numberToHangul(9999)).toBe('구천구백구십구');
  });

  test('다양한 변환 케이스', () => {
    expect(numberToHangul(10_000)).toBe('일만');
    expect(numberToHangul(15_000)).toBe('일만오천');
    expect(numberToHangul(123_456)).toBe('십이만삼천사백오십육');
    expect(numberToHangul(987_654_321)).toBe('구억팔천칠백육십오만사천삼백이십일');
    expect(numberToHangul(1_000_000)).toBe('백만');
  });
});
