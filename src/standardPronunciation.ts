...
import { hasProperty, isArrayIncludes, isNotUndefined } from './utils';
  ...
  for (const hangulPhrase of hangulPhrases) {
    ...
    for (let i = 0; i < disassembleHangul.length; i += 1) {
      ...
      if (options.hardConversion && nextSyllable) {
        ...
        */
        if (isArrayIncludes(된소리_받침_23항, currentSyllable.last) && hasProperty(된소리, nextSyllable.first)) {
        ...
        */
        if (
          isArrayIncludes(어간_받침, currentSyllable.last) &&
      ...
      */
      if (i > 0 && currentSyllable.last && nextSyllable?.first === 음가가_없는_자음) {
        ...
        if (isArrayIncludes(특별한_한글_자모, combinedSyllables)) {
          const 다음_음절의_초성 =
          ...
          continue;
        } else if (isArrayIncludes(한글_자모, combinedSyllables)) {
      ...
      */
      if (
        ...
        isArrayIncludes(ㄴㄹ이_덧나는_모음, nextSyllable.middle)
      ) {
        if (isArrayIncludes(ㄴㄹ이_덧나서_받침_ㄴ_변환, currentSyllable.last)) {
          if (currentSyllable.last === 'ㄱ') {
            currentSyllable.last = 'ㅇ';
          }

          nextSyllable.first = 'ㄴ';
        } else if (isArrayIncludes(ㄴㄹ이_덧나서_받침_ㄹ_변환, currentSyllable.last)) {
      ...
      */
      if (isArrayIncludes(자음동화_받침_ㄴ_변환, currentSyllable.last) && nextSyllable?.first === 'ㄹ') {
        nextSyllable.first = 'ㄴ';
      }
      ...
      */
      if (currentSyllable.last && nextSyllable && ['ㄴ', 'ㅁ'].includes(nextSyllable.first)) {
        if (isArrayIncludes(비음화_받침_ㅇ_변환, currentSyllable.last)) {
          currentSyllable.last = 'ㅇ';
        } else if (isArrayIncludes(비음화_받침_ㄴ_변환, currentSyllable.last)) {
          currentSyllable.last = 'ㄴ';
        } else if (isArrayIncludes(비음화_받침_ㅁ_변환, currentSyllable.last)) {
    ...
    */
      ...
      if (currentSyllable.last) {
        if (isArrayIncludes(발음변환_받침_ㅎ, currentSyllable.last)) {
          if (nextSyllable) {
          ...
          isArrayIncludes(발음변환_첫소리_ㅎ, currentSyllable.last) &&
          nextSyllable &&
